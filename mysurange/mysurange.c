...
int main(int argc, char **argv){
	// Check IO
	if(argc!=(1+1)){
		printf("\n MYSURANGE.EXE - fast surange (1st & last hdrs)\n");
		printf("\n mysurange.exe file.su\n\n");
		exit(0);
	}
	...
	// Read 1st trace
	fread(&hdr, 1, TRCBYTES, fpin);
	
	// Read last trace
	int offset = TRCBYTES + hdr.ns*sizeof(float);
	fseek(fpin, -offset, SEEK_END);
	fread(&hdr2, 1, TRCBYTES, fpin);
	
	//MYSURANGE
	printf("%d traces:\n", hdr2.tracl);
	printf("tracl\t%d %d (%d - %d)\n", hdr.tracl, hdr2.tracl, hdr.tracl, hdr2.tracl);
	printf("fldr\t%d %d (%d - %d)\n", hdr.fldr, hdr2.fldr, hdr.fldr, hdr2.fldr);
	printf("tracf\t%d %d (%d - %d)\n", hdr.tracf, hdr2.tracf, hdr.tracf, hdr2.tracf);
	printf("trid\t%d\n", hdr.trid);
	printf("sdepth\t%d\n", hdr.sdepth);
	printf("scalel\t%d\n", hdr.scalel);
	printf("scalco\t%d\n", hdr.scalco);
	printf("sx\t%d\n", hdr.sx);
	printf("sy\t%d\n", hdr.sy);
	printf("gx\t%d %d (%d - %d)\n", hdr.gx, hdr2.gx, hdr.gx, hdr2.gx);	
	printf("gy\t%d %d (%d - %d)\n", hdr.gy, hdr2.gy, hdr.gy, hdr2.gy);
	printf("ns\t%d\n", hdr.ns);	
	printf("dt\t%d\n", hdr.dt);
	printf("trwf\t%d\n", hdr.trwf);		
	printf("d1\t%5.8f\n", hdr.d1);
	printf("d2\t%5.8f\n", hdr.d2);
	printf("ntr\t%d %d (%d - %d)\n", hdr.ntr, hdr2.ntr, hdr.ntr, hdr2.ntr);	
